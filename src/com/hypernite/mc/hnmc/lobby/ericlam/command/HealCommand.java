...
public class HealCommand extends SettingsCommandNode {
    ...
    public void executeSettings(Player name, CommandSender sender, MessagesConfig var) {
        ...
        if (hp < maxHealth || hg < 20){
            ...
            name.sendMessage(var.getCommandMSG().getHeal().get("active"));
            if (name != sender){
                sender.sendMessage(var.getCommandMSG().getHeal().get("Be-active").replace("<player>", name.getName()));
            }
        } else {
            sender.sendMessage(var.getCommandMSG().getHeal().get(name == sender ? "no-need" : "Be-no-need").replace("<player>", name.getName()));
        }
    }
}
